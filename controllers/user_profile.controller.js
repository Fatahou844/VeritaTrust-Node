...
exports.create = function(req, res) {
    ...
    if(req.body.constructor === Object && Object.keys(req.body).length === 0){
        ...
        user_profile.create(new_user_profile, function(err, user_profile) {
          if (err)
          res.send(err);
          res.json({error:false,message:"user_profile added successfully!",data:user_profile});
        });
    }
};
...
exports.update = function(req, res) {
    ...
    if(req.body.constructor === Object && Object.keys(req.body).length === 0){
        ...
        user_profile.update(req.params.id, new user_profile(req.body), function(err, user_profile) {
            if (err)
                res.send(err);
            res.json({ error:false, message: 'user_profile successfully updated' });
       });
    }
};
...
exports.delete = function(req, res) {
    user_profile.delete( req.params.id, function(err, user_profile) {
      if (err)
      res.send(err);
      res.json({ error:false, message: 'user_profile successfully deleted' });
    });
};