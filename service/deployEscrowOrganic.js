...
const deployEscrowContract = async (deployContent, connection, job_address) => {
    ...
    try {
      ...
      /*  connection.query("UPDATE organic_merchant_review SET status = 'published' where status = 'pending' and 	jobId = ?",job_address, function (err, result) {
            if (err) {
                err;
            }
        ...
        connection.query("UPDATE organic_merchant_review SET hash_transaction = ?, status = 'published'  where jobId = ?",[hashfinal.transactionHash, job_address], function (err, result) {
            if (err) {
                err;
            }
            console.log(result.affectedRows + " record(s) hash transaction and status updated");
        });
        
         
        // Creation de la requette pour insérer un nouveau reward sur le compte du client
        connection.query("INSERT INTO reward (Reward_type, User_id, Job_id,reward_value) VALUES (?,(SELECT organic_merchant_review.user_id FROM organic_merchant_review WHERE organic_merchant_review.jobId = ?),?,?)", ['merchant_review',job_address, job_address, 1], function (err, res) {
                    if(err) {
