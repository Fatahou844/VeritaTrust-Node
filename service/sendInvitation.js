...
const emailInvitation = nodeCron.schedule("*/30 * * * * *", function cronJob() {
  ...
  try {
    const sql =
      "SELECT * FROM invitations where Delivery_status = 'Not delivered' and invitation_type = 'merchant_review' and has_sent != 1  LIMIT 1";
    db.sequelize
          ...
          <td class="bg_white unsubscribe" style="text-align: center;">
              ...
              translationTemplateEmaling[customer_merchant_language][
                ...
                smtpTemplate.subject =
                  "Concerning your order " + userMap[element.id].domaine_name;
                ...
                apiInstance.updateSmtpTemplate(templateId, smtpTemplate).then(
                  function () {
                    ...
                    db.sequelize
                      .query(sql_update1, { type: sequelize.QueryTypes.UPDATE })
                      .then((results) => {
                        console.log(results);
                        // res.json(results);
                      });
                    ...
                    db.sequelize
                      .query(sql_update2, { type: sequelize.QueryTypes.UPDATE })
                      .then((results) => {
                        console.log(results);
                        // res.json(results);
                      });
