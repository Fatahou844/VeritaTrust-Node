...
const { createuserTransaction, getuserTransaction, updateuserTransaction } = require("../controllers/userTransaction.controller");

const isAuthenticated = (req, res, next) => {
  if (req.isAuthenticated()) {
    // L'utilisateur est authentifié, continuez avec la prochaine étape de la route
    
     if (req.body.user_id != req.user.id && req.params.userId != req.user.id) {
...
router.put("/",isAuthenticatedAdmin, updateuserTransaction);
router.post("/",passport.authenticate("jwt", { session: false }),verifyAccess, createuserTransaction);
router.get("/getbyuserid/:userId", passport.authenticate("jwt", { session: false }),verifyAccess, getuserTransaction)
