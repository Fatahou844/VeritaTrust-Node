...
passport.use(
  ...
  new LocalStrategy(
    ...
    async (req, email, password, done) => {
      try {
        const user = await VeritatrustUsers.findOne({
          where: {
            [Op.and]: [{ emailAddress: email }, { emailAddress: email }],
          },
        });
...
router.post("/userprofile/resetpassword", async (req, res, next) => {
    ...
    resetPasswordNotif(userdaata.first_name, userdaata.last_name, confirmation_link, email);
    //res.clearCookie("connect.sid"); // This logs out the user.
    res.send("email-confirmation-update");
