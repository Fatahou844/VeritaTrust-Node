...
const {
  create,
  findAll
} = require('./controllers/merchant_review.controller');
const {
  create2
...
app.use(
  helmet.contentSecurityPolicy({
    directives: {
      // Autres directives...
      "img-src": ["'self'", "data:", "*"],
      "script-src": ["'self'", "https://cdn.jsdelivr.net","https://js.hcaptcha.com","https://unpkg.com","https://code.jquery.com","https://api.veritatrust.com","*"],
      "connect-src": ["'self'", "https://newassets.hcaptcha.com", "https://accounts.google.com","https://api.cloudinary.com/v1_1/dnbpmsofq/image/upload","https://api.cloudinary.com/v1_1/drbhco8al/image/upload","http://dev.veritatrust.com"],
      "frame-src":["'self'", "https://newassets.hcaptcha.com","https://accounts.google.com/o/oauth2/auth"],
    },
  })
); 
...
app.get('/api/burak', (req, res) => {
  ...
  db.sequelize.query(`UPDATE product_review SET status = 'published' where status = 'moderation'`, { type: sequelize.QueryTypes.UPDATE }).then(results => {
            console.log(results);
        // res.json(results);
        });
